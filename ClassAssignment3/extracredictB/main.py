...
motion = []
count = 0
offsetList = np.empty((0,3),float)
global narrList, varrList, iarrList, varr_List,smoothNarrList
...
class Bvh:
    ...
    def drawObj(self):
        global isObj,narrList, varrList,smoothNarrList,varr_List,iarrList,count,offsetList
        
        count = 0
     
        for (n,joint) in self.joints.items() :
        

            glTranslatef(joint.offset[0], joint.offset[1], joint.offset[2])

         
            offset = np.sqrt(joint.offset[0]**2+joint.offset[1]**2+joint.offset[2]**2)
                
            degree = np.cross(normalized(joint.offset),np.array([0,1,0]))
            degreeSize = np.rad2deg(np.arcsin(np.sqrt(degree[0]**2+degree[1]**2+degree[2]**2)))
            if np.dot(joint.offset, np.array([0,1,0])) > 0:
                degreeSize = 180 - degreeSize
            glPushMatrix()
            glRotatef(degreeSize,degree[0],degree[1],degree[2])
            ...
            glTranslatef(offsetList[count][0],offsetList[count][1],offsetList[count][2])
#            print(n+"--------")
#            print(count)
#                if(count == 5):
#                    glRotatef(-90,0,0,1)
            drawObject(narrList[count],varrList[count],smoothNarrList[count],varr_List[count],iarrList[count])
    ...
    def drawObjAnimate(self, frameNumber):
        global motion, isBox,offsetList
        ...
        for (n,joint) in self.joints.items() :
            ...
            for motion_idx, motion_type in zip(joint.channelIdx, joint.channelOrder):
                if motion_type.lower() == 'xrotation':
                    glRotatef(motion[frameNumber][motion_idx], 1, 0, 0)
                elif motion_type.lower() == 'yrotation':
                    glRotatef(motion[frameNumber][motion_idx], 0, 1, 0)
...
#        glScalef(.01,.01,.01)
    ...
    def animateMotion(self):
        global pressTime,anim,positionMatrixf,isObj
...
#        glScalef(.01,.01,.01)
        ...
        if(isObj == True):
            self.drawObjAnimate(frameNumber)
        else:
...
def key_callback(window,key,scancode, action, mods):
    global isPerspective,isHierarchicalMode,iswireframeMode,isSmoothShading,isAnimate,pressTime,isBox,isObj
...
def loadObjFile():
    global narrList, varrList, iarrList, varr_List,smoothNarrList,path,offsetList
    ...
    path = 'hip.obj'
    tempNarr,tempVarr,tempSmoothNarr,tempVarr_,tempIarr = ObjRendering()
    narrList.append(tempNarr)
    varrList.append(tempVarr)
    smoothNarrList.append(tempSmoothNarr)
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.,0.,0.]]),axis=0)
    ...
    offsetList = np.append(offsetList,np.array([[0.,-1.7,0.]]),axis=0)
    ...
    offsetList = np.append(offsetList,np.array([[0.,-2.5,0.]]),axis=0)
    ...
    offsetList = np.append(offsetList,np.array([[1.,0.,0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[.2,-3.,0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.2,1,0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.1,1.,0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.,7.,-1.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[-0.05,-0.,1.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.03,-0.0,0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[0.,0.0,-0.]]),axis=0)
    ...
    varr_List.append(tempVarr_)
    iarrList.append(tempIarr)
    offsetList = np.append(offsetList,np.array([[-0.,-3,0.0]]),axis=0)
