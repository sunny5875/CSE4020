...
currentEndPoint = np.array([0,0,0,1])
...
#
    ...
    glPopMatrix()
    beforeEndPoint = currentEndPoint
    currentEndPoint = J2 @ T1 @ np.array([0,0,0,1])
    difference = (currentEndPoint - beforeEndPoint) * 10
    

        
    glBegin(GL_LINES)
    glColor3ub(255, 0, 0)
    glVertex3fv(currentEndPoint[:3])
    glVertex3fv((currentEndPoint+difference)[:3])
    glEnd()
